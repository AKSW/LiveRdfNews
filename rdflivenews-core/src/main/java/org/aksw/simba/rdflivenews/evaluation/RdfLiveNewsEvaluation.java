 ...
 * 
 */
public class RdfLiveNewsEvaluation {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for ( Boolean forceTyping : Arrays.asList(/*true, */false)  ) {
            for ( String refinementType : Arrays.asList("ALL"/*, "PERSON", "NONE"*/)) {
...
//        List<Pattern> top1PercentPattern = patternsOfIteration.size() > 100000 ? patternsOfIteration.subList(0, 1000) : patternsOfIteration.subList(0, patternsOfIteration.size() / 100);
        ...
        // use this only if you want to compare against the old gold standard
        List<Pattern> top1PercentPattern = new ArrayList<Pattern>();
        for ( Pattern p : patternsOfIteration ) if ( p.isAboveThresholds() ) top1PercentPattern.add(p);
        
        System.out.println(top1PercentPattern.size());
        ...
        System.out.println("similarity done");
